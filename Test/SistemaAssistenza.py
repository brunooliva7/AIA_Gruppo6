...
def elabora_linee_guida(frame, blu_lower, blu_upper):
    ...
    global diramazione_vista_prima  # Flag: stavamo già vedendo 2+ rami PRIMA di perdere la linea
    ...
    # Condizione necessaria: ci sono 2+ contorni E il secondo è abbastanza grande
    # rispetto al primo (evita falsi positivi da riflessi o oggetti blu casuali).
    if len(contorni_validi) >= 2:
        ...
        # Il secondo ramo deve essere almeno il 25% del ramo principale
        # per essere considerato un vero bivio e non un riflesso/oggetto spurio
        if area_secondaria >= area_principale * 0.25:
            incrocio_rilevato = True
            diramazione_vista_prima = True   # Memorizziamo che stavamo vedendo un bivio
        # Se il secondo contorno è troppo piccolo rispetto al primo, non è un incrocio
    ...
    if len(contorni_validi) > 0:
        ...
        if moments["m00"] > 0:
            ...
            # Aggiorna da che lato si stava spostando la linea rispetto al centro
            if errore_x > 0:
        ...
        # Un incrocio reale si riconosce perché:
        #   1. Avevamo già visto la diramazione (2+ rami) nei frame precedenti
        #   2. La linea è appena sparita (vuoto centrale dell'incrocio)
        # Se la linea è sparita per un altro motivo (camera alzata, fine percorso,
        # spostamento brusco) la flag diramazione_vista_prima è False → LINEA PERSA normale.
        if (contatore_linea_persa >= SOGLIA_FRAME_LINEA_PERSA
            ...
            incrocio_rilevato = False
            # Se la linea è persa senza diramazione precedente, azzeriamo la flag
            # così non rimane "sporca" per il prossimo tratto
            if contatore_linea_persa >= SOGLIA_FRAME_LINEA_PERSA:
...
def gestisci_macchina_a_stati(errore_x, incrocio_rilevato, cartelli_disponibili,larghezza):
    ...
    if stato_attuale == STATO_NAVIGAZIONE:
        if incrocio_rilevato:
            ...
            #LINEA VISIBILE
            fase_recupero_annunciata = 0  # Reset della fase di recupero se la linea è visibile
            tolleranza = 180  # Pixel di tolleranza per evitare indicazioni troppo frequenti
...
# ----------------------------------------------------
        ...
        # Calcola la direzione opposta: se la linea era a destra del centro,
        # l'utente si è spostato verso sinistra → deve tornare verso destra.
        ...
        if contatore_linea_persa >= SOGLIA_FRAME_LINEA_PERSA and fase_recupero_annunciata < 1:
            ...
            if dir_recupero:
                invia_voce(
                    f"Attenzione, linea persa. L'ultima posizione era a {dir_recupero}. "
