...
ultimo_tempo_voce = 0
INTERVALLO_VOCE = 4.0  # Secondi di silenzio obbligatori tra indicazioni di linea ripetitive (aumentato per dare più respiro)
...
def thread_bocca_tts_unico():
    ...
    while dati_condivisi["running"]:
        try:
            elemento = coda_voce_unica.get(timeout=0.5)
            ...
            try:
                # Invia il messaggio all'interfaccia web (SitoWebDialogoVoice.py)
                requests.post("http://127.0.0.1:5000/api/invia-messaggio", json={"testo": testo, "taglia_audio": taglia_audio}, timeout=2)
                
                # Simula il tempo di pronuncia sul telefono per mantenere la sincronizzazione (circa 1 sec ogni 10 caratteri)
                tempo_lettura = max(1.0, len(testo) / 10.0)
                
                # Invece di time.sleep, usa wait su un evento. Così se arriva un messaggio prioritario, lo interrompe SUBITO.
                interruzione_voce.wait(timeout=tempo_lettura)
                interruzione_voce.clear()  # Resetta l'evento per la prossima volta

            except Exception as e:
                print(f"[ERRORE] Invio messaggio al web fallito: {e}")
            
            # Finito di parlare, la bocca segnala "verde" alle orecchie
            semaforo_voce.set()    # Verde: il telefono ha "finito", l'orecchio può ascoltare
...
# Evento per interrompere il "time.sleep" del TTS e forzare il cambio frase immediato
interruzione_voce = threading.Event()

# 3. Aggiorna la funzione invia_voce per usare la nuova coda
def invia_voce(testo, prioritario=False, interrompi_subito=False):
    global ultimo_messaggio_navigazione, ultimo_tempo_voce
    tempo_attuale = time.time()

    if prioritario:
        if interrompi_subito:
            interruzione_voce.set()  # Sveglia immediatamente il thread se sta dormendo
        ...
        # Mettiamo in coda il messaggio. Il taglia_audio viene inviato al telefono SOLO se interrompi_subito=True
        coda_voce_unica.put((testo, interrompi_subito))
        ultimo_messaggio_navigazione = ''
        ultimo_tempo_voce = 0
        ...
        if testo != ultimo_messaggio_navigazione or (tempo_attuale - ultimo_tempo_voce) > INTERVALLO_VOCE:
            ...
            # --- LOGICA: più di 2 messaggi in coda -> svuota tutto e tieni solo l'ultimo ---
            coda_overflow = False
            if coda_voce_unica.qsize() > 2:
                coda_overflow = True
                while not coda_voce_unica.empty():
                    try:
                        coda_voce_unica.get_nowait()
            ...
            coda_voce_unica.put((testo, coda_overflow))
...
def gestione_comandi_tastiera(tasto, monitoraggio_attivo, ultimo_annuncio_ambiente, da_voce=False):
    """
    ...
    if tasto == ord('w') or tasto == ord('W'):
        if not monitoraggio_attivo:
            ...
            if not da_voce:
                invia_voce("Monitoraggio ambientale attivato. Metti il telefono parallelo al petto.", prioritario=True, interrompi_subito=True)
    ...
    # Controllo se viene premuto 'S' (sia minuscolo che maiuscolo)
        ...
        if monitoraggio_attivo:
            ...
            if not da_voce:
                invia_voce("Monitoraggio ambientale disattivato. Metti il telefono parallelo al pavimento. Torno alla navigazione.", prioritario=True, interrompi_subito=True)
            print("[SISTEMA] Monitoraggio Ambientale Continuo: SPENTO")
            
    # Controllo se viene premuto 'Q' (sia minuscolo che maiuscolo)
    elif tasto == ord('q') or tasto == ord('Q'):
        continua_ciclo = False
        print("[SISTEMA] Ricevuto comando di TERMINAZIONE. Uscita in corso...")
            
    return monitoraggio_attivo,ultimo_annuncio_ambiente,continua_ciclo


# ====================================
# 5. THREAD POLLING COMANDI WEB
# ====================================
def thread_polling_comandi():
    global comando_remoto
    import requests
    print("[THREAD POLLING] In ascolto di comandi dall'interfaccia web...")
    while dati_condivisi["running"]:
        try:
            r = requests.get("http://127.0.0.1:5000/api/leggi-comando", timeout=1)
            ...
            for testo, direzione in cartelli_disponibili.items():
                if direzione in ["DESTRA", "SINISTRA", "DRITTO"]:
                    ...
                    if direzione == "SINISTRA":
                        invia_voce(f"Ho letto {testo}. Fai un passo in avanti e ruota a sinistra sul posto di novanta gradi.", prioritario=True)
                    elif direzione == "DESTRA":
                        invia_voce(f"Ho letto {testo}. Fai un passo in avanti e ruota a destra sul posto di novanta gradi.", prioritario=True)
                    else: # DRITTO
...
def main():
    ...
    threading.Thread(target=thread_polling_comandi, daemon=True).start()
    ...
    while video.isOpened():
        ...
        if monitoraggio_ambientale_attivo:
            ...
            if tempo_corrente - ultimo_annuncio_ambiente > 8.0:
                ...
                if not labels_viste:
                    ...
                    if oggetti_descritti:
                        if len(oggetti_descritti) > 1:
                            lista_oggetti = ", ".join(oggetti_descritti[:-1]) + " e " + oggetti_descritti[-1]
                        else:
                            lista_oggetti = oggetti_descritti[0]
                        descrizione = "Nell'ambiente rilevo: " + lista_oggetti + "."
                    else:
                        descrizione = "Non riesco a stimare la distanza degli oggetti rilevati."
                    
                    # Usa prioritario=True così svuota i vecchi messaggi di navigazione e parla sùbito
                    invia_voce(descrizione, prioritario=True, interrompi_subito=True)

                # Diamo tempo al thread TTS di recepire il messaggio e far scattare il semaforo rosso
                time.sleep(0.1) 
                semaforo_voce.wait()
                ultimo_annuncio_ambiente = time.time()
            
            # (Rimosso il reset distruttivo della coda che bloccava l'audio)
        ...
        # Sovrascrive il tasto se è arrivato un comando vocale remoto
        da_voce = False
        if comando_remoto:
            if comando_remoto in ['W', 'W_VOICE']:
                tasto = ord('w')
                da_voce = True
            elif comando_remoto in ['S', 'S_VOICE']:
                tasto = ord('s')
                da_voce = True
            elif comando_remoto in ['Q', 'Q_VOICE']:
                tasto = ord('q')
                da_voce = True
            comando_remoto = None
            
            if da_voce:
                with coda_voce_unica.mutex:
                    coda_voce_unica.queue.clear()
        ...
        monitoraggio_ambientale_attivo, ultimo_annuncio_ambiente,continua = gestione_comandi_tastiera(
            tasto, monitoraggio_ambientale_attivo, ultimo_annuncio_ambiente, da_voce
        )
