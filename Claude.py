...
SOGLIA_FRAME_LINEA_PERSA = 15    # Numero di frame senza linea prima di attivare l'allarme smarrimento
    ...
    with microfono as source:
        ...
        while True:
            ...
            if WAKE_WORD in testo_sveglia:
                print("\n [TRIGGER -ASSISTENTE] Parola d'ordine intercettata!")
                ...
                try:
                    ...
                    invia_voce("Non ho sentito nulla. Annullamento.", prioritario=True)
                    semaforo_voce.wait() 
                    esegui_bip_chiusura() 
                    
                except sr.UnknownValueError:
                    print("[STT ERROR] Audio incomprensibile.")
                    invia_voce("Non sono riuscito a decifrare l'audio.", prioritario=True)
                    semaforo_voce.wait()
                    esegui_bip_chiusura()
                    
                except sr.RequestError as e:
                    print(f"[ERRORE RETE] Impossibile contattare Google: {e}")
                    invia_voce("Errore di rete.", prioritario=True)
                    semaforo_voce.wait()
                    esegui_bip_chiusura()
...
def elabora_linee_guida(frame, blu_lower, blu_upper):
    ...
    if len(contorni_validi) > 0:
        ...
        if moments["m00"] > 0:
            ...
            # Se la linea è visibile, resettiamo il contatore di perdita e aggiorniamo l'ultima posizione valida
        ...
        if contatore_linea_persa >= SOGLIA_FRAME_LINEA_PERSA and ultima_x_valida is not None:
            incrocio_rilevato = True 
        else:
            incrocio_rilevato = False
...
def gestisci_macchina_a_stati(errore_x, incrocio_rilevato, cartelli_disponibili,larghezza):
    global stato_attuale, direzione_da_prendere, tempo_inizio_svolta,mappa_cartelli_global,contatore_linea_persa
    ...
    if stato_attuale == STATO_NAVIGAZIONE:
        if incrocio_rilevato:
            ...
            if errore_x is not None:
                tolleranza = 100  # Pixel di tolleranza per evitare indicazioni troppo frequenti
                ...
                if contatore_linea_persa >= SOGLIA_FRAME_LINEA_PERSA:
                    if ultima_x_valida is not None:
                        # Se l'ultimo punto noto era a sinistra del centro, la linea è uscita a sinistra.
                        # Significa che l'utente si è girato troppo a destra: deve ruotare a sinistra per riallinearsi.
                        if ultima_x_valida < centro_camera:
                            invia_voce("Percorso smarrito. Ruota di circa trenta gradi verso sinistra per riprendere la linea.", prioritario=True)
                        # Se era a destra, la linea è uscita a destra. L'utente deve ruotare a destra.
                        else:
                            invia_voce("Percorso smarrito. Ruota di circa trenta gradi verso destra per riprendere la linea.", prioritario=True)
                    else:
                        # Cassa di ripiego se non c'è una storia precedente memorizzata
                        invia_voce("Attenzione, linea non rilevata. Cammina lentamente cercandola.", prioritario=False)
                else:
                    # In attesa di confermare lo smarrimento oltre la soglia dei frame di tolleranza
                    pass
    ...
    while video.isOpened():
        ...
        if not coda_comandi_utente.empty():
            ...
            if "ambiente" in comando or "intorno" in comando:
                ...
                invia_voce("Arresto immediato.", prioritario=True)
                semaforo_voce.wait()
                break
                
            elif "grazie" in comando or "annulla" in comando or "chiudi" in comando:
                invia_voce("Va bene, disattivo il microfono.", prioritario=True)
                semaforo_voce.wait()
                esegui_bip_chiusura()
                
            else:
                invia_voce("Non ho capito, puoi ripetere?", prioritario=True)
                semaforo_voce.wait()
                esegui_bip_chiusura()
        ...
        gestisci_macchina_a_stati(errore_x, incrocio_rilevato, cartelli_disponibili,larghezza=frame.shape[1]) # Passa anche la larghezza del frame per le decisioni di centraggio e riallineamento
